...
const PromptSubmit: React.FC<PromptSubmitProps> = ({list, setList, isloading, setIsLoading}) => {
    ...
    return (
        <div className='absolute bottom-0 w-full mb-1'>
            <form onSubmit={submitHandler} className='relative'>
                ...
                <input className='w-full py-3 pl-2 pr-[50px] outline-none rounded-b-sm' onChange={(e) => setPrompt(e.target.value)} type="text" placeholder='Enter prompt' />
                <button disabled={isloading} type='submit' className='absolute right-4 bottom-0 top-0'><SubmitIcon /></button>
            </form>
        </div>
    )
}
